...
args.add_argument("-c", "--config", help="Configuration file", default="Configurations_PbPb")
...
if initialization:
    ...
    log_dir = os.path.join(out_dir, "Conf_And_Logs")
    os.makedirs(log_dir, exist_ok=True)
    logger_file = os.path.join(log_dir, f"Run_{datetime.now().strftime('%Y-%m-%d')}.log")
    logger = logger_config(log_path = logger_file, log_name = "Running Control")
    logger.info("")
    logger.info("-"*50)
    logger.info("This is where the analyszer started!")
    logger.info(f"Task Name: {config.Analysis['Task_Name']}, Analysis doing:")
    if config.Doing["Cut_Variations"]:
        logger.info(f"Fraction by cut-variation: {config.Analysis['Name_fraction']}")
    if config.Doing["Data_And_Efficiency"]:
        logger.info(f"Data writing: {config.Analysis['Name_writing']}")
    if config.Doing["Signal_Extraction"]:
        logger.info(f"Raw yield extraction: {config.Analysis['Name_massfit']}")
    if config.Doing["Rho_Extraction"]:
        logger.info(f"Rho extraction: {config.Analysis['Name_rhoextract']}")
    logger.info("-"*50)
    logger.info("Creating work dir and copy configuration file to it....")

    os.system(f"cp {config.__file__} {log_dir}/Configurations_{datetime.now().strftime('%Y-%m-%d')}.py -f")
...
if config.Doing["Cut_Variations"]:
    ...
    if config.Doing["Rewriting_for_frac"]:
...
if config.Doing["Data_And_Efficiency"]:
    ...
    if config.EP_Resolution["Redoing"]:
        data_ops.write_EP_resolution()
    data_ops.write_data(data)
    data_ops.write_mc(mc)
    if config.Template_fit:
